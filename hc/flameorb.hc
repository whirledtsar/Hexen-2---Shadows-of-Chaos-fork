...
void flamestream_touch ()
{
	...
	if(other.takedamage)
	{
		...
		sound(self,CHAN_BODY,"succubus/flamend.wav",1,ATTN_NORM);	//0.5
...
//ws: play fire crackling sounds from flame stream
void flamestream_soundoff ()
{
	sound(self,CHAN_BODY,"raven/kiltorch.wav",0.5,ATTN_NORM);
	remove(self);
}

void flamestream_sound (vector org, string snd)
{
	entity newent;
	newent = spawn();
	...
	float vol;
	if (snd == "misc/fburn_sm.wav")
		vol = 1;
	else
		vol = 0.5;
	
	sound(newent,CHAN_BODY,snd,vol,ATTN_NORM);
	
	newent.think = flamestream_soundoff;
	thinktime newent : random(0.8,1.5);
}
...
void flamestream_anim()
{
	...
	else 
	{
		...
		{			
			...
			if (rand < 0.33) {
				if (random()<0.3)	flamestream_sound(trace_endpos, "misc/fburn_sm.wav");
				starteffect(CE_FIREWALL_SMALL, trace_endpos, '0 0 0', 0);
			}
			else if (rand < 0.66) {
				if (random()<0.3)	flamestream_sound(trace_endpos, "raven/flame1.wav");
				starteffect(CE_FIREWALL_MEDIUM, trace_endpos, '0 0 0', 0);
			}
			else {
				if (random()<0.3)	flamestream_sound(trace_endpos, "misc/fburn_bg.wav");
				starteffect(CE_FIREWALL_LARGE, trace_endpos, '0 0 0', 0);
			}
		}
		else
	...
	/*if (self.t_length < time) {
		sound(self,CHAN_BODY,"misc/fburn_bg.wav",0.5,ATTN_NORM);
		self.t_length=time+0.5;
	}*/
...
void flamestream_fire ()
{
	...
	sound(newmis,CHAN_BODY,"misc/fburn_bg.wav",1,ATTN_NORM);
